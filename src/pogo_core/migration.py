...
from pogo_core.util import topologicalsort
...
class Migration:
    ...
    @property
    def depends(self) -> set[Migration]:
        return self._depends or set()

    @property
    def use_transaction(self) -> bool:
        return self._use_transaction

    @property
    def depends_ids(self) -> set[str]:
    ...
    async def apply(self, db: asyncpg.Connection) -> None:
        if self._apply is None:
            return
        await self._apply(db)

    async def rollback(self, db: asyncpg.Connection) -> None:
