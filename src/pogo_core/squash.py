...
def parse(statement: str, logger: Logger | logging.Logger | None = None) -> ParsedStatement:
    logger = logger or logger_
    ...
    logger.debug("parsed tokens: %s", parsed.tokens)
...
def parse_sqlglot(statement: str, logger: Logger | logging.Logger | None = None) -> ParsedStatement:
    logger = logger or logger_
    try:
    ...
    if isinstance(parsed, (exp.Create, exp.Alter, exp.Drop)):
        ...
        identifier = ident.name if ident and ident.this is not None else str(ident).replace('"', "")
